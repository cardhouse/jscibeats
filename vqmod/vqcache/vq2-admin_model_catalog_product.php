                    ...
                    if (isset($data['product_option'])) {
                     	foreach ($data['product_option'] as $product_option) {
                                 if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                                         if (isset($product_option['product_option_value'])  && count($product_option['product_option_value']) > 0 ) {
                                                 foreach ($product_option['product_option_value'] as $product_option_value) {
                                                  if($product_option_value['download_id'] > 0){   
                                                     $this->db->query("REPLACE INTO " . DB_PREFIX . "product_to_download SET 
                                                         product_id = '" . (int)$product_id . "', 
                                                         download_id = '" . (int)$product_option_value['download_id'] . "'");
                                                    }
                                                 }
                                         }
                                 }					
...
$this->db->query("DELETE FROM " . DB_PREFIX . "mta_product WHERE product_id = '" . (int)$product_id . "' and price_mod_type='special'");//+mod by yp
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
                    ...
                    'download_id'           => $product_option_value['download_id']
	...
	public function getTotalProducts($data = array()) {
                    ...
                    $table = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "product_option_value WHERE Field='download_id'");
                    if ($table->num_rows==0) {
                    $this->db->query("ALTER TABLE `" . DB_PREFIX . "product_option_value` ADD `download_id` INT NOT NULL AFTER `weight_prefix`");
                    }
