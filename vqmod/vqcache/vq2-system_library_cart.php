...
$option_download = 0;
                   ...
                   if ($option_value_query->row['download_id'] > 0) {
                   $option_download = $option_value_query->row['download_id'];
                   }
                   ...
                   'download_id'           => $option_value_query->row['download_id']
...
$this->aff_info['products'][$product_id]['mods']['special'] = $product_special_query->row['product_special_id'];//+mod by yp
                   ...
                   $option_download = array();
                   foreach ($option_data as $value) {
                   $option_download[]=$value["download_id"];
                   }
                   if($option_download){ 
                   $download_option = array();
                   $download_option = $this->db->query("SELECT * FROM " . DB_PREFIX . "download d LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) WHERE d.download_id IN(" . implode(",", $option_download) . ") AND dd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                   $download_option_data = array();
                   foreach ($download_option->rows as $download) {
                   $download_option_data[] = array(
                   'download_id' => $download['download_id'],
                   'name'        => $download['name'],
                   'filename'    => $download['filename'],
                   ...
                   $download_data = array_merge($download_data, $download_option_data);
                   }
