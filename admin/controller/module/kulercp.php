...
class ControllerModuleKulercp extends Controller {
    ...
    public function index() {
        ...
        $this->data['action'] = HTTPS_SERVER . 'index.php?route=module/kulercp&token=' . $this->session->data['token'];
    ...
    {
        $this->data['downloadable']                 = version_compare(VERSION, '1.5.5.1', '==');

        $this->data['download_sample_data_url']     = $this->helperLink('module/kulercp/sampleDataUrl');
        $this->data['check_modules_url']            = $this->helperLink('module/kulercp/checkkulermodules');
        $this->data['upload_installation_url']      = $this->helperLink('module/kulercp/installsampledata');
    }
    ...
    public function sampleDataUrl()
    {
        $json = array();

        if ($sample_data_base_url = $this->getBaseSampleDataUrl())
        {
            $json['status']         = 1;
    ...
    public function checkKulerModules()
    {
        $this->language->load('module/kulercp');

        $json = array();
        $demo_url = $this->getBaseSampleDataUrl();
    ...
    private function getBaseSampleDataUrl()
    {
...
//        $url = 'http://dev.dc.localhost/oc1551/';
	...
	private function saveAction() {
        ...
        if(isset($blocks['payment']['items']) && is_array($blocks['payment']['items'])) {
            ...
            foreach ($blocks['payment']['items'] as $extension => & $item) {
                ...
                $text_link = $this->language->get('text_' . $extension);
                if ($text_link != 'text_' . $extension) {
                    $link = str_replace('view/image/payment/', 'admin/view/image/payment/', $this->language->get('text_' . $extension));
                } else {
                    $link = '';
                }
        ...
        $this->model_kuler_cp->editSetting('kulercp', 'color', $color, $store_id);
        $this->model_kuler_cp->editSetting('kulercp', 'seo', $seo, $store_id);
        $this->model_kuler_cp->editSetting('kulercp', 'font', $font, $store_id);
        $this->model_kuler_cp->editSetting('kulercp', 'kulercp_design', $design, $store_id);

        // Save custom CSS
        if ($design['status'])
        {
            $this->compileCustomCss($design['colors']);
        }
    ...
    private function compileCustomCss(array $colors)
    {
        ...
        foreach ($design_options as $section_label => $section_properties)
        {
            ...
            {
                $property = $section_label . '_' . $section_property;
                if (!isset($colors[$property]))
                {
                    $colors[$property] = '';
                }

                if ($property_type == 'image' && trim($colors[$property]) && utf8_strpos($colors[$property], 'http') !== 0)
                {
                    $colors[$property] = $catalog_base . $colors[$property];
                }
            }
        }

        $dir_template = DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/';
        $path = $dir_template . 'includes/custom.php';
        $css = '';
        ...
        return file_put_contents($dir_template . 'stylesheet/custom.css', $css);
    }
