...
<div class="kuler-css3-slideshow <?php echo $data['dimension']; ?> <?php echo $data['bullet'] ? 'bullet' : ''; ?>">
...
<script>
    $(function () {
        ...
        var type = '<?php echo $data['dimension']; ?>',
			defaultWidth = <?php echo isset($data['default_width']) ? $data['default_width'] : 0; ?>,
            width = <?php echo $data['width']; ?>,
            height = <?php echo $data['height']; ?>,
            screens = <?php echo json_encode($data['screen']); ?>,
            otherHeights = <?php echo json_encode($data['other_height']); ?>,
        ...
        $window.on('resize', function () {
            ...
            if (type == 'fluid') {
                if ((screens[0] && windowWidth > screens[0]) || windowWidth > 1024) {
                    rHeight = height;
                }

                for (var i = 0; i < screens.length - 1; i++) {
                    if (screens[i] && windowWidth > screens[i + 1] && windowWidth <= screens[i]) {
                        rHeight = otherHeights[i];
                    }
                }

                if (screens[i] && windowWidth <= screens[i]) {
                    rHeight = otherHeights[i];
                }
